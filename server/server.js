...
app.post('/post/question', (req, res) => {
  ...
  const date = '111';
  const sqlQuery =
    'INSERT INTO post (TITLE, CONTENT, AUTHOR, DATE) VALUES (?,?,?,?)';
...
app.post('/accept/answer', (req, res) => {
  ...
  const sqlAnswerQuery =
    "UPDATE answer SET ACCEPTED = 'true' WHERE idanswer= ? ";
...
app.get('/accept/answer/:idpost', (req, res) => {
  ...
  const sqlAnswerQuery =
    "SELECT * FROM answer WHERE idpost = ? AND ACCEPTED = 'true' ";
...
app.post('/post/answer', (req, res) => {
  ...
  const sqlQuery =
    'INSERT INTO answer (ANSWER, AUTHOR, DATE, idpost) VALUES (?,?,?,?)';
